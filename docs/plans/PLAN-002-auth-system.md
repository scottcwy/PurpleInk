...
## 0-bis. 现状核销（2026-07-28 追加）

本计划撰写于阶段 A 落地前，§0.1「仓库没有任何认证」等表述已过时。截至
2026-07-28：**阶段 A（身份/会话/页面/邮件）与阶段 B（API 守卫、页面查库校验、
队列与业务查询的 workspace 归属收口、归属迁移脚本、AI 登录门）均已完成**，
登录页体验账号提示弹窗已按用户决策移除（建号机制保留）。当前真值见
`docs/conventions/routing.md` §9 与 `docs/issues/ISSUE-015-production-issue.md` §12；
本文其余内容保留为设计依据与历史记录，遇冲突以上述两处为准。
