> **2026-07-29 结构化错误与 Provider 限流升级（当前口径）**
>
> 本轮确认“切换其他模型后链路完美、StepFun 批量失败”的本质不是分镜合同或模型兼容性，
> 而是把本机 `director-stage` 并发误当成供应商 API 额度。当前 StepFun 账户按 **5 RPM**
> 配置；并发、RPM、TPM 分开建模。旧链路没有共享凭据级出网预留，429 又进入普通重试和
> 熔断计数，因此一次限流会被放大为整批节点失败。
>
> 已完成四个边界清晰的修复阶段：
>
> 1. `f74afe1`：建立 `WorkflowFault schemaVersion=2`，按 origin / recovery / funding
>    结构化归因；Provider 原始 body、Prompt、凭据和工具参数不进入持久化或浏览器。
> 2. `93c6c16`：增加 PostgreSQL Provider 调度层，按托管共享凭据或 BYOK 指纹原子预留
>    并发/RPM/TPM；429 按 `Retry-After` 或滚动窗口延后，不计普通重试、失败预算和熔断。
> 3. `2ef24f1`：Streaming Log、Inspector 和错误对话框统一消费安全投影；等待态保持
>    pending，显示倒计时、切换模型与取消等待，不标红、不自动弹失败框、不建议跳过。
> 4. `3c0700b`：修正 Provider 调度迁移 journal 的顺序，使此前被 Drizzle 静默跳过的
>    `provider_dispatches / provider_dispatch_cooldowns` 两张表真实落库；迁移连续双跑，
>    并增加 journal 单调性回归测试。
> 5. `55afd88`：统一使用 PostgreSQL 时钟创建、判断和释放 Provider 租约，修复应用进程
>    与数据库时钟漂移时并发租约可能提前失效的问题。
> 6. 当前文档与验收阶段：模式 M 固化上述事故、边界与验收规则；StepFun 专项已用真实
>    7 镜头项目观察到滚动 60 秒最大 5 次出网。按当前测试约定，完整链路改用 Gemini，
>    终片 Artifact、HTTP 下载和 SHA-256 证据完成后补写最终结果。
>
> 单元与合同覆盖包括 400/401/402/403/404/408/413/422/429/451/5xx、网络/超时、
> BYOK 与平台托管归因、v1 兼容、安全投影、恢复动作白名单；PostgreSQL 覆盖 5 RPM、
> 429 defer、重启恢复、托管预算共享、15 分钟终态和取消等待。真实 Chromium 已确认旧错误
> 能被新 UI 正确投影、技术详情默认折叠且控制台无错误。此前“UI 端测由用户自行进行”的旧结论
> 已失效，本轮由代理完成。
>
> StepFun 专项项目 `fab81e59-6a86-444a-9b2a-067cc86b139d` 生成了 39 个节点（4 个全局节点
> 加 7 组镜头节点），`provider_dispatches.reserved_at` 的真实 SQL 窗口统计为 17 次出网、
> 任意滚动 60 秒最大 5 次。首轮端测曾误把媒体路由一并切到 StepFun，造成旁白失败；该结果
> 已明确作废，验收脚本现只切文本与视觉路由，媒体保持原配置。后续常规与完整端测默认 Gemini，
> StepFun 只用于 429、限流与兼容性专项验证。
>
> **2026-07-29 运行时补充结论（历史口径）**
>
...
> 重新 FABRICATE。下游字幕端测还发现 ASR 毫秒时长未按整秒计费原子归一化，现已
> 对预留与实际结算统一向上取整。提交前检查以
> `docs/conventions/workflow-failure-patterns.md` 模式 I/J/K/L 为准。
