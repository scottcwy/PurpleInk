...
### 实测数据（2026-07-25，详细方法与原始数据见 `evidence/issue-012/baseline-metrics.md`）

目标项目 39 节点（4 全局 + 7 泳道 × 5），121.1s 活跃观测窗口：

| 指标 | 实测值 |
| --- | --- |
| `getCanvasGraph()` 调用 | 249 次（**2.06 次/秒**，dev 下单次 refresh 触发多次渲染） |
| 单次耗时 | p50 23.1ms / p95 60.5ms / max 111.5ms |
| 每次调用 SQL | 41 条（N+2：节点 + 边 + 每节点 artifacts），窗口内累计 ≈ 10,209 条 |
| DB 变更 → 下一次画布查询 | 7 样本：p50 **1359ms**，max **1949ms**（不满足 <1s） |
| 主线程 long task | 137.7s 内 22 次，累计 1200ms（无交互，纯轮询重渲染） |

另实测到结构性缺口：页面加载时若无 pending/running 节点，轮询 effect 永不启动，
后续由 API/其它会话触发的节点活动在 UI 上完全不可见。

**决策：选定方案 A**。方案 B 退避轮询最坏延迟 3–5s 不达标；方案 C 被上表否定。
注：受 SHOT_SPEC 上游缺陷（`validate_shot_plan` 工具输出提取失败）限制，
本轮测量窗口由失败节点循环重入队维持；全链路（含 render 泳道）对比待上游修复后补。
