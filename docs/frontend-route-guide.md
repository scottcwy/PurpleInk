...
> 文档用途：说明 PurpleInk 各前端路由的产品职责、页面内容、核心操作和页面之间的关系。  
> 适用对象：产品经理、设计师、前端工程师、后端工程师和测试人员。  
> 当前基准：以 `docs/specs/2026-07-23-product-flow-launch-video-system.md` 的 Accepted v1 路由为准。

## 1. PurpleInk 的路由在解决什么问题

PurpleInk 不是一个自由编辑视频的工具，而是一套持续制作产品发布视频的工作流。用户先登记一个长期存在的 Product，再为某次功能上线创建一条 Release，最后把经过批准的发布信息和真实产品证据制作成视频 Artifact。

三个核心对象的区别如下：

| 对象 | 含义 | 示例 |
| --- | --- | --- |
| Product | 长期存在、可重复发布的软件产品 | PurpleInk、Notion、Linear |
| Release | 围绕某次功能发布开展的一次视频制作流程 | “AI 搜索功能发布” |
| Artifact | Release 最终产生的交付文件 | 横版 MP4、竖版 MP4、封面图、质量报告 |
...
路由中的 `:releaseId` 是动态参数，表示当前操作的是哪一条 Release。例如：
...
| --- | --- | --- |
| `/login` | 登录已有账号 | 保留 |
| `/signup` | 注册并创建 Workspace | 保留 |
| `/dashboard` | Workspace 工作总览 | 保留 |
| `/products` | Product 列表和创建入口 | 保留 |
| `/releases` | Release 列表和创建入口 | 保留 |
| `/releases/:releaseId/brief` | 定义本次发布要讲什么 | 保留 |
| `/releases/:releaseId/sources` | 旧版素材来源页 | 已由 `/flow` 和 `/evidence` 取代 |
| `/releases/:releaseId/storyboard` | 编排视频叙事和分镜 | 保留 |
| `/releases/:releaseId/review` | 审阅预览并反馈或批准 | 保留 |
| `/releases/:releaseId/render` | 旧版独立渲染页 | 已移除 |
| `/releases/:releaseId/artifacts` | 查看任务进度和最终交付物 | 保留 |
...
| 当前步骤 | 继续下一步的主要条件 | 下一步 |
| --- | --- | --- |
| Brief | ReleaseBriefVersion 已批准 | Flow |
| Flow | 已固定批准的 ProductFlowVersion | Evidence |
| Evidence | 必需节点通过且 Evidence 已批准 | Storyboard |
| Storyboard | StoryboardVersion 已批准 | Review |
| Review | Preview 通过检查且已批准 | Artifacts |
| Artifacts | 所需终稿全部发布 | Release complete |
...
| 页面 | 它负责什么 | 它不负责什么 |
| --- | --- | --- |
| Login / Signup | 身份和 Workspace 建立 | Product 或 Release 制作 |
| Dashboard | 汇总和导航 | 深度编辑 |
| Products | 长期产品资产 | 某一次发布的制作状态 |
| Releases | Release 列表和入口 | ProductFlow 资产维护 |
| Brief | 发布目标和内容合同 | 录制产品、编辑视频 |
| Flow | 选择可复用产品操作路径 | 审片和交付 |
| Evidence | 执行路径并确认真实证据 | 决定视频叙事顺序 |
| Storyboard | 组织 Scene 和证据 | 自由时间线剪辑 |
| Review | 审阅预览、反馈和批准 | 管理长期 Product 资产 |
| Artifacts | 后台任务状态和最终交付 | 修改已批准 Storyboard |

## 8. 当前代码实现情况

截至本文编写时：

- `/login`、`/signup`、`/dashboard`、`/products` 和 `/releases` 已有基础页面结构。
- Release 的正式路由已经建立：`brief`、`flow`、`evidence`、`storyboard`、`review`、`artifacts`。
- Release 顶部步骤导航已经具备完成、当前和锁定状态设计。
- Release 具体页面内容尚未接通，目前共享渲染入口返回 404。
- `/sources` 和 `/render` 不属于当前正式路由，不应据此新增页面实现。

本文解释的是每个路由应承担的最终产品职责，不表示上述交互均已在当前前端中实现。
