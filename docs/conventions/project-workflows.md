...
| `audio` | 用户录音 | ASR → 原音频时间轴 → 复用文稿分镜与渲染，不重复 TTS | `planned` |
| `website` | 可访问 URL | Playwright 取证 → 网站介绍编排 → 复用成熟生视频系统 | `planned` |
...
- `POST /api/projects` 的目标合同是按 `kind` 判别的联合输入。
- 当前阶段仅 `script` 创建已接线；audio / website 不得伪装成 script 写入。
...
audio / website 的执行计费尚未接线。在接线前不得展示已扣费、固定成功率或伪造成本。
网站成熟 worker 内部调用应收敛成可审计的工作流服务用量，不能把原始凭据、prompt
或供应商错误暴露到项目 UI。
