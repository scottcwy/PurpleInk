...
## 7.34 模式 AN：单个零时长 ASR 边界字使长录音整体退化为单分镜

**症状**：长音频已成功转写，`shot-split` 也显示成功，但画布只有一个超长 `S001`。
真实样本约 455 字、93.6 秒，StepFun ASR 返回数百个时间片，其中少量单字满足
`startMs === endMs`；旧时间轴校验因此丢弃全部可用时间戳，退回
`whole-recording → U001 → S001`。

**规则与护栏**：只允许把“文字非空、零时长、且起止点恰好等于前一个合法时间片
结束点”的边界字并入前片，保持文字顺序和全文不丢失。无法安全归属的零时长片、重叠、
倒序、越界或缺失时间戳继续 fail-closed 为整段录音。归一化只位于音频导入专用的
`user-audio-timeline`，不得修改三来源拓扑、Director、队列、路由、计费或 Artifact
合同。回归测试锁定时间戳模式、连续采样覆盖和全文守恒。
...
| AN ASR 零时长边界字 | 已通过全文守恒与时间轴回归 | 不适用 | 待真实长音频复测 | 回归通过，验收待完成 |
