...
describe('租约与执行超时常量', () => {
  ...
  it('按 kind 给执行超时：导出与异步旁白均保留 30 分钟合同', () => {
    ...
    expect(EXECUTION_TIMEOUT_MS['audio-transcription']).toBe(1_800_000)
    expect(EXECUTION_TIMEOUT_MS['website-video']).toBe(3_000_000)
    ...
    expect(executionTimeoutMs('audio-transcription')).toBe(1_800_000)
    expect(executionTimeoutMs('website-video')).toBe(3_000_000)
  ...
  it('租约覆盖该 kind 的完整执行窗口与一次清扫间隔，不能在合法执行中途回收', () => {
    ...
    expect(leaseDurationMs('audio-transcription')).toBe(1_860_000)
    expect(leaseDurationMs('website-video')).toBe(3_060_000)
