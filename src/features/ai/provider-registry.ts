...
export const PROVIDER_REGISTRY: Record<AiProviderId, ProviderDefinition> = {
  gemini: catalogDefinition('gemini'),
  stepfun: catalogDefinition('stepfun'),
  mimo: catalogDefinition('mimo'),
  openai: catalogDefinition('openai'),
  anthropic: catalogDefinition('anthropic'),
...
function catalogDefinition(providerId: BuiltInProviderId): ProviderDefinition {
  ...
  const defaultModels: Partial<Record<ProviderCapability, string>> = {}
  for (const capability of capabilities) {
    const deployment = deployments.find((candidate) =>
      candidate.capabilities.some((item) => item === capability))
    if (deployment) defaultModels[capability] = deployment.logicalModelId
  }
  return {
    id: providerId,
