...
#
# 未启用 `output: 'standalone'`（§2.4 首版推荐）：整份 node_modules 进运行镜像，
# 体积更大但零风险，且迁移用的 `tsx` 天然可用。
...
RUN npx --yes playwright@"$(node -p "require('./node_modules/playwright/package.json').version")" install-deps chromium
...
USER pwuser
RUN npx --yes playwright@"$(node -p "require('./node_modules/playwright/package.json').version")" install chromium

ENV NODE_ENV=production
EXPOSE 3000
# 直接跑 next 包的真实入口，不走 node_modules/.bin/next——那是 pnpm 生成的
# POSIX shell shim（#!/bin/sh 开头），`node <shim>` 会把 shell 代码当 JS
# 解析报语法错误；shim 设计上是给 shell 直接执行的，不是给 `node` 当参数喂。
