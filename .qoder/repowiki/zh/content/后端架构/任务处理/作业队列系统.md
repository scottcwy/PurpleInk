...
- **新增**：增强的尝试管理系统，在发送等待前覆盖旧尝试，创建不消耗常规重试配额的新尝试，防止释放的计费调用被重用
...
**最新更新**：队列系统现已支持音频转录作业处理，通过新增的音频转录队列处理器实现了高效的音频文件转录功能。同时引入了尝试绑定机制，为计费集成提供了精确的用量追踪能力。幂等性处理的增强确保了系统的可靠性和一致性。新增的专用车道机制为长时间运行任务提供了更好的隔离和资源管理。**最新增强**：尝试管理系统已得到显著改进，现在能够在发送等待前覆盖旧尝试，创建不消耗常规重试配额的新尝试，有效防止释放的计费调用被重用，提升了系统的资源管理和计费准确性。
...
- **增强**：增强的尝试管理系统：在发送等待前覆盖旧尝试，创建不消耗常规重试配额的新尝试，防止释放的计费调用被重用

**更新**：队列系统现已集成音频转录功能和计费集成。新增的音频转录队列处理器支持高效的音频文件转录，而管理式音频计费服务提供了精确的用量统计和费用计算能力。所有组件现在都支持工作空间上下文，确保在多租户环境中正确隔离数据访问。新增的专用车道机制为长时间运行任务提供了更好的资源隔离和性能保障。**最新增强**：尝试管理系统得到了显著改进，现在能够在发送等待前覆盖旧尝试，创建不消耗常规重试配额的新尝试，有效防止释放的计费调用被重用，提升了系统的资源管理和计费准确性。
...
- **增强**：尝试管理层：统一管理作业尝试的生命周期，防止计费调用重用

**更新**：队列系统现已集成音频转录功能和计费集成。新增的计费层提供了精确的用量统计和费用计算能力，确保音频转录服务的准确计费。专用车道机制为长时间运行任务提供了更好的资源隔离。**最新增强**：尝试管理层得到了显著改进，能够有效防止释放的计费调用被重用，提升了系统的资源管理和计费准确性。
...
participant AttemptManager as "尝试管理器"
...
TranscriptionHandler->>AttemptManager : 创建新尝试(覆盖旧尝试)
AttemptManager-->>TranscriptionHandler : 返回新尝试ID
...
**更新**：任务调度器现已集成新的队列声明机制，支持更健壮的队列配置管理和默认值处理。任务执行现在在runInAuthContext(SYSTEM_USER_ID)中进行，确保所有操作都在系统用户权限下执行，提高安全性和一致性。**最新增强**：任务调度器集成了增强的尝试管理系统，能够在发送等待前覆盖旧尝试，创建不消耗常规重试配额的新尝试，有效防止释放的计费调用被重用。
...
### 增强的尝试管理系统（Enhanced Attempt Management）
职责与行为
- 尝试覆盖机制：在发送等待前自动覆盖旧尝试，确保尝试的唯一性和时效性
- 独立配额管理：创建的新尝试不消耗常规重试配额，避免配额滥用
- 计费调用保护：防止释放的计费调用被重用，确保计费的准确性和公平性
- 生命周期管理：统一管理作业尝试的创建、执行、完成和清理
- 上下文传递：确保工作空间上下文在执行过程中保持一致
- 资源清理：自动清理临时资源和释放占用的系统资源
- 状态同步：实时更新任务状态到存储层

**新增**：增强的尝试管理系统是本次更新的核心功能之一，专门解决计费调用重用问题和尝试管理的复杂性。它能够在发送等待前覆盖旧尝试，创建不消耗常规重试配额的新尝试，有效防止释放的计费调用被重用。

关键特性
- 尝试覆盖：自动覆盖旧尝试，确保尝试的唯一性
- 独立配额：新尝试不消耗常规重试配额
- 计费保护：防止释放的计费调用被重用
- 生命周期管理：完整的尝试生命周期管理
- 上下文一致性：确保工作空间上下文的一致性
- 资源清理：自动清理临时资源
- 状态同步：实时更新任务状态

应用场景
- 音频转录：防止重复转录同一音频文件
- 外部API调用：避免重复调用已释放的API配额
- 长时间任务：确保长时间任务的尝试唯一性
- 计费敏感操作：保护计费调用的准确性和公平性
...
**更新**：尝试完成模块已与新队列声明机制集成，提供更好的尝试生命周期管理和资源清理支持。**最新增强**：尝试完成模块现在集成了增强的尝试管理系统，能够在发送等待前覆盖旧尝试，创建不消耗常规重试配额的新尝试，有效防止释放的计费调用被重用。
...
+overrideOldAttempts()
+createNewAttemptWithoutQuota()
+preventBillingInvocationReuse()
}
...
- **新增**：尝试管理优化：覆盖旧尝试的机制减少了不必要的重试和计费调用，提高了整体效率

**更新**：新队列声明机制提供了更高效的配置解析和验证，减少了初始化时间和内存占用。新增的音频转录功能通过批量处理和智能缓存优化了性能。计费服务的异步处理避免了主流程阻塞，提高了整体吞吐量。专用车道机制为长时间运行任务提供了更好的性能保障。**最新增强**：增强的尝试管理系统通过覆盖旧尝试和创建不消耗常规配额的新尝试，显著减少了不必要的重试和计费调用，提高了系统的整体效率和资源利用率。
...
- **新增**：尝试管理问题：检查尝试覆盖机制是否正常工作，确认旧尝试是否被正确覆盖
...
**最新更新**：队列系统已成功集成音频转录功能和计费集成。新增的音频转录队列处理器提供了高效的音频文件转录能力，而管理式音频计费服务确保了准确的用量统计和费用计算。**最新增强**：增强的尝试管理系统显著提升了系统的资源管理和计费准确性，通过覆盖旧尝试和创建不消耗常规配额的新尝试，有效防止了释放的计费调用被重用。这些改进显著提升了系统的功能完整性和商业价值，为用户提供了更加丰富和可靠的音频处理能力。专用车道机制的引入进一步增强了系统对长时间运行任务的支持能力。
...
- **新增**：尝试管理配置：尝试覆盖策略、配额管理规则、计费调用保护设置
...
- 尝试管理扩展：自定义尝试生命周期和资源管理策略，支持尝试覆盖和配额管理
...
- 尝试操作：创建和管理作业尝试，处理生命周期事件，支持尝试覆盖和配额管理
...
- 尝试监控：尝试创建成功率、平均执行时间、资源清理率、尝试覆盖成功率
...
### 尝试完成模块详解
尝试完成模块现在集成了增强的尝试管理系统：

- 生命周期管理：统一管理作业尝试的创建、执行、完成和清理
- 上下文传递：确保工作空间上下文在执行过程中保持一致
- 资源清理：自动清理临时资源和释放占用的系统资源
- 状态同步：实时更新任务状态到存储层
- **新增**：尝试覆盖机制：在发送等待前覆盖旧尝试，确保尝试的唯一性
- **新增**：独立配额管理：创建不消耗常规重试配额的新尝试
- **新增**：计费调用保护：防止释放的计费调用被重用
