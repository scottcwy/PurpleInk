...
## 更新摘要
**所做更改**   
- 增强了 subtitle.ts 和 subtitle-ass.ts 的音频处理和字幕验证功能
- 新增品牌词漂移校正机制，提高字幕与音频的同步精度
- 实施更严格的拒绝逻辑，提升字幕质量控制的严格性
- 优化了时间轴对齐算法，减少品牌词在语音中的位置偏差
- 改进了错误处理机制，增强系统的稳定性和容错能力
...
**更新** 新增了品牌词漂移校正和更严格的拒绝逻辑，显著提升了字幕生成的准确性和可靠性。
...
- 字幕导出器（export-subtitles）：统一处理正常和降级导出场景，提供容错和重试机制。
- 音频格式（audio-format）：统一音频格式识别与兼容性处理。

**更新** subtitle.ts 和 subtitle-ass.ts 现在包含增强的品牌词漂移校正和更严格的验证逻辑。
...
## 架构总览
下图展示从"文本→TTS→音频→时间轴→字幕→ASS导出"的主流程，强调新增的品牌词漂移校正和严格验证机制对时间轴对齐精度的提升。
...
- 内聚性
  ...
  - export-subtitles 作为独立的导出管理层，解耦具体的导出逻辑。
...
- 常见问题
  ...
  - 导出失败：检查 export-subtitles 的错误日志和降级状态。
- **更新** 新增问题类型
  - 品牌词漂移：检查漂移校正算法的参数设置和识别准确性。
  - 严格拒绝：查看拒绝日志，了解具体拒绝原因和修复建议。
- 诊断步骤
  - 查看各阶段日志与指标；回放关键时间戳区间。
  - 使用单元测试覆盖边界用例（空文本、超长段落、特殊字符）。
  - 逐步隔离问题域（媒体→度量→字幕→导出→导出器）。
  - 检查导出器的降级策略是否被触发。
  - **更新** 重点检查品牌词漂移校正和严格验证的日志输出。
- 恢复策略
  - 失败重试与降级（切换TTS、简化特效、缩短分片）。
  - 保留中间产物以便复现与对比。
  - 利用导出器的自动重试和恢复机制。
  - **更新** 调整品牌词漂移校正参数和严格验证阈值。
...
## 结论
PurpleInk的字幕子系统以清晰的层次与职责划分，实现了从文本到ASS字幕的高质量生成。通过精确的时间轴对齐、灵活的文本分割、完善的ASS样式与特效支持，以及健壮的错误处理与性能优化策略，能够在多语言与复杂场景下稳定输出可用字幕。**更新** 新增的品牌词漂移校正和严格拒绝逻辑进一步提升了字幕生成的准确性和可靠性，确保在各种环境下都能稳定产出高质量字幕。建议在生产环境中结合监控与自动化测试，持续保障质量与稳定性。
...
- 最佳实践
  ...
  - 导出策略：合理配置降级阈值，平衡质量和可用性。
  - **更新** 品牌词处理：正确配置品牌词列表，优化漂移校正参数。
- 兼容性处理
