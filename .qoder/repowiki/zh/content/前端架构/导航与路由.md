...
## 核心组件
- 应用壳与侧边栏
  - app-shell：应用级外壳，承载顶部栏、侧边栏与主内容区
  - app-sidebar-shell：侧边栏容器，负责展开/收起、模式切换
  - app-sidebar：侧边栏菜单渲染与交互
  - nav-context：导航上下文，统一当前路由、跳转、权限判断
  - products-routes：产品域路由表，驱动菜单生成与权限过滤
...
## 架构总览
Next.js App Router 以目录为路由，配合路由组与 layout/template/loading/error 等约定文件，形成清晰的层级与职责分离。导航层通过 features/navigation 提供统一的菜单、权限与高亮能力；API 路由提供数据获取与操作入口。
...
subgraph "导航系统"
Shell["应用壳<br/>app-shell.tsx"]
SidebarShell["侧边栏容器<br/>app-sidebar-shell.tsx"]
Sidebar["侧边栏<br/>app-sidebar.tsx"]
NavCtx["导航上下文<br/>nav-context.tsx"]
Routes["路由表<br/>products-routes.ts"]
end
subgraph "API 路由"
...
AppSidebar --> NavContext : "读取当前路径/权限"
...
Shell --> SidebarShell["app-sidebar-shell.tsx"]
SidebarShell --> Sidebar["app-sidebar.tsx"]
Sidebar --> NavCtx["nav-context.tsx"]
NavCtx --> Routes["products-routes.ts"]
...
## 结论
本项目基于 Next.js App Router 构建了清晰、可扩展的导航与路由体系。通过路由组与布局隔离、集中式导航上下文与路由表、完善的错误与加载态处理，实现了良好的用户体验与可维护性。建议在后续迭代中持续优化懒加载与预取策略，完善测试覆盖，确保路由行为的稳定性与性能。
