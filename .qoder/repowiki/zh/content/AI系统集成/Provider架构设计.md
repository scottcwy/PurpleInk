...
## 架构总览
下图展示了从请求进入、路由决策、Provider调用到结果返回的整体流程，以及配置与Profile存储的交互。
...
Router->>Adapter : "执行调用(携带Payload)"
Adapter->>Store : "读取认证/偏好(Profile)"
...
## 结论
PurpleInk的Provider架构通过统一接口抽象、动态注册与适配器模式，实现了高度可扩展与可维护的AI服务集成体系。配置系统与路由策略保证了灵活性与稳定性，Profile存储提供了安全的凭据管理能力。遵循本文档的扩展指南，可以快速接入新的AI提供商并保持系统一致性。
...
- 步骤概览：
  ...
  5. 测试验证：单元测试覆盖Payload构造、错误处理与边界条件。
- 最佳实践：
  - 保持适配器无状态，依赖注入外部资源。
  - 使用OpenAI兼容层减少重复实现。
  - 严格校验输入输出，避免脏数据传播。
