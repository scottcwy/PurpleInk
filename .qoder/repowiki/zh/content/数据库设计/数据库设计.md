# 数据库设计

<cite>
**本文引用的文件**   
- [drizzle.config.ts](file://drizzle.config.ts)
- [scripts/migration/export-sqlite.ts](file://scripts/migration/export-sqlite.ts)
- [scripts/migration/import-postgres.ts](file://scripts/migration/import-postgres.ts)
- [scripts/migration/provision-master-key.ts](file://scripts/migration/provision-master-key.ts)
- [scripts/migration/reconcile-postgres.ts](file://scripts/migration/reconcile-postgres.ts)
- [scripts/migration/sqlite-backup.ts](file://scripts/migration/sqlite-backup.ts)
- [scripts/setup/db-migrate.ts](file://scripts/setup/db-migrate.ts)
- [scripts/setup/postgres-init.sql](file://scripts/setup/postgres-init.sql)
- [scripts/setup/seed-owner-account.ts](file://scripts/setup/seed-owner-account.ts)
- [src/features/auth/account-service.pg.test.ts](file://src/features/auth/account-service.pg.test.ts)
- [src/features/auth/auth-repository.ts](file://src/features/auth/auth-repository.ts)
- [src/features/auth/verification-repository.ts](file://src/features/auth/verification-repository.ts)
- [src/features/canvas/queries.ts](file://src/features/canvas/queries.ts)
- [src/features/director/runtime-repository.pg.test.ts](file://src/features/director/runtime-repository.pg.test.ts)
- [src/features/director/runtime-repository.ts](file://src/features/director/runtime-repository.ts)
- [src/features/render/cache.pg.test.ts](file://src/features/render/cache.pg.test.ts)
- [src/features/render/repository.pg.test.ts](file://src/features/render/repository.pg.test.ts)
- [src/features/routing/media-route-repository.pg.test.ts](file://src/features/routing/media-route-repository.pg.test.ts)
- [src/features/routing/model-route-repository.pg.test.ts](file://src/features/routing/model-route-repository.pg.test.ts)
- [src/features/billing/domain.ts](file://src/features/billing/domain.ts)
- [src/features/billing/contracts.ts](file://src/features/billing/contracts.ts)
- [src/features/billing/rate-card.ts](file://src/features/billing/rate-card.ts)
- [src/features/billing/redemption.ts](file://src/features/billing/redemption.ts)
- [src/features/billing/period-service.ts](file://src/features/billing/period-service.ts)
- [src/features/billing/ledger.ts](file://src/features/billing/ledger.ts)
- [src/features/billing/rate-card-repository.ts](file://src/features/billing/rate-card-repository.ts)
</cite>

## 更新摘要
**所做更改**
- 新增计费系统数据库架构章节，涵盖工作空间权限、计费周期、费率卡、合同和兑换交易表
- 扩展实体关系图以包含计费相关实体
- 添加计费数据访问层设计与Repository模式实现
- 更新性能考虑部分以包含计费查询优化策略
- 增强故障排查指南以覆盖计费相关问题的诊断

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [计费系统数据库架构](#计费系统数据库架构)
7. [依赖关系分析](#依赖关系分析)
8. [性能考虑](#性能考虑)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
11. [附录](#附录)

## 简介
本文件面向PurpleInk项目的数据库设计与数据访问层，聚焦PostgreSQL作为持久化存储，结合Drizzle ORM进行Schema定义、迁移与查询构建。文档涵盖实体关系图、表结构与索引策略、Repository模式与连接池管理、事务处理、数据完整性约束、性能优化（查询优化、索引设计、分库分表）、以及备份恢复与灾难恢复方案。内容基于仓库中现有脚本与测试用例所体现的数据库使用方式进行分析与总结，帮助读者快速理解并扩展系统的数据层能力。

**更新** 本次更新重点介绍了新加入的计费系统数据库架构，包括工作空间权限管理、计费周期控制、费率卡配置、合同管理和兑换交易记录等核心功能模块。

## 项目结构
与数据库相关的关键位置包括：
- Drizzle配置与迁移脚本位于根目录与scripts目录下
- PostgreSQL初始化SQL位于scripts/setup/postgres-init.sql
- 数据导入导出与备份脚本位于scripts/migration
- 各业务模块通过Repository访问数据库，并在pg测试中验证行为
- **新增** 计费系统相关文件位于src/features/billing/目录下

```mermaid
graph TB
A["应用代码<br/>features/*"] --> B["Repository层<br/>auth, director, render, routing, billing"]
B --> C["Drizzle ORM<br/>Schema/查询构建"]
C --> D["PostgreSQL<br/>实例/连接池"]
E["迁移与初始化<br/>drizzle.config.ts, db-migrate.ts"] --> C
F["导入/导出/备份<br/>import-postgres.ts, sqlite-backup.ts"] --> D
G["计费系统<br/>billing/*"] --> H["计费Repository<br/>rate-card-repository.ts"]
H --> C
```

图表来源
- [drizzle.config.ts](file://drizzle.config.ts)
- [scripts/setup/db-migrate.ts](file://scripts/setup/db-migrate.ts)
- [scripts/migration/import-postgres.ts](file://scripts/migration/import-postgres.ts)
- [scripts/migration/sqlite-backup.ts](file://scripts/migration/sqlite-backup.ts)
- [src/features/billing/rate-card-repository.ts](file://src/features/billing/rate-card-repository.ts)

章节来源
- [drizzle.config.ts](file://drizzle.config.ts)
- [scripts/setup/db-migrate.ts](file://scripts/setup/db-migrate.ts)
- [scripts/setup/postgres-init.sql](file://scripts/setup/postgres-init.sql)
- [scripts/migration/import-postgres.ts](file://scripts/migration/import-postgres.ts)
- [scripts/migration/sqlite-backup.ts](file://scripts/migration/sqlite-backup.ts)
- [src/features/billing/rate-card-repository.ts](file://src/features/billing/rate-card-repository.ts)

## 核心组件
- Drizzle ORM配置与迁移入口
  - drizzle.config.ts：集中配置数据库连接、驱动、迁移与生成规则
  - scripts/setup/db-migrate.ts：执行迁移与种子数据的统一入口
- 数据访问层（Repository）
  - auth-repository.ts、verification-repository.ts：认证与验证码相关数据访问
  - runtime-repository.ts：导演运行时状态持久化
  - render相关repository：渲染任务、缓存、缩略图等
  - routing相关repository：媒体路由与模型路由
  - **新增** rate-card-repository.ts：费率卡数据访问
- 迁移与数据工具
  - import-postgres.ts：从外部源导入到PostgreSQL
  - export-sqlite.ts：导出SQLite用于离线或轻量场景
  - sqlite-backup.ts：SQLite备份脚本
  - provision-master-key.ts：主密钥准备（安全敏感数据）
  - reconcile-postgres.ts：PostgreSQL一致性校验与修复
- 初始化脚本
  - postgres-init.sql：数据库对象初始化（用户、权限、扩展等）
  - seed-owner-account.ts：初始账户种子数据
- **新增** 计费系统核心组件
  - domain.ts：计费领域模型定义
  - contracts.ts：计费API契约定义
  - rate-card.ts：费率卡业务逻辑
  - redemption.ts：兑换交易处理
  - period-service.ts：计费周期服务
  - ledger.ts：计费账本管理

章节来源
- [drizzle.config.ts](file://drizzle.config.ts)
- [scripts/setup/db-migrate.ts](file://scripts/setup/db-migrate.ts)
- [scripts/setup/postgres-init.sql](file://scripts/setup/postgres-init.sql)
- [scripts/setup/seed-owner-account.ts](file://scripts/setup/seed-owner-account.ts)
- [scripts/migration/import-postgres.ts](file://scripts/migration/import-postgres.ts)
- [scripts/migration/export-sqlite.ts](file://scripts/migration/export-sqlite.ts)
- [scripts/migration/sqlite-backup.ts](file://scripts/migration/sqlite-backup.ts)
- [scripts/migration/provision-master-key.ts](file://scripts/migration/provision-master-key.ts)
- [scripts/migration/reconcile-postgres.ts](file://scripts/migration/reconcile-postgres.ts)
- [src/features/auth/auth-repository.ts](file://src/features/auth/auth-repository.ts)
- [src/features/auth/verification-repository.ts](file://src/features/auth/verification-repository.ts)
- [src/features/director/runtime-repository.ts](file://src/features/director/runtime-repository.ts)
- [src/features/billing/domain.ts](file://src/features/billing/domain.ts)
- [src/features/billing/rate-card-repository.ts](file://src/features/billing/rate-card-repository.ts)

## 架构总览
整体数据流遵循"应用层 → Repository → Drizzle ORM → PostgreSQL"的分层结构。迁移与初始化在部署阶段执行，确保Schema与数据一致；运行期通过连接池复用连接，提升并发性能；关键路径采用事务保证一致性。

```mermaid
sequenceDiagram
participant App as "应用服务"
participant Repo as "Repository"
participant Drizzle as "Drizzle ORM"
participant DB as "PostgreSQL"
App->>Repo : 调用数据方法
Repo->>Drizzle : 构建查询/事务
Drizzle->>DB : 执行SQL
DB-->>Drizzle : 返回结果
Drizzle-->>Repo : 类型化结果
Repo-->>App : 业务对象
```

图表来源
- [src/features/auth/auth-repository.ts](file://src/features/auth/auth-repository.ts)
- [src/features/director/runtime-repository.ts](file://src/features/director/runtime-repository.ts)
- [src/features/billing/rate-card-repository.ts](file://src/features/billing/rate-card-repository.ts)
- [drizzle.config.ts](file://drizzle.config.ts)

## 详细组件分析

### Drizzle ORM与迁移管理
- 配置与驱动
  - drizzle.config.ts定义了数据库连接参数、驱动选择与迁移输出目录
- 迁移执行
  - scripts/setup/db-migrate.ts负责执行迁移与种子数据，确保环境一致性
- Schema与查询
  - 各Repository通过Drizzle API定义表结构与查询条件，避免手写SQL带来的不一致风险

```mermaid
flowchart TD
Start(["开始"]) --> ReadConfig["读取Drizzle配置"]
ReadConfig --> ApplyMigrations["执行迁移"]
ApplyMigrations --> SeedData["执行种子数据"]
SeedData --> Ready(["就绪"])
```

图表来源
- [drizzle.config.ts](file://drizzle.config.ts)
- [scripts/setup/db-migrate.ts](file://scripts/setup/db-migrate.ts)

章节来源
- [drizzle.config.ts](file://drizzle.config.ts)
- [scripts/setup/db-migrate.ts](file://scripts/setup/db-migrate.ts)

### 认证与账号数据访问
- 认证Repository
  - auth-repository.ts封装用户、会话、密码重置等数据操作
  - verification-repository.ts管理验证码生命周期与校验
- 事务与一致性
  - 登录、注册、密码重置等流程通常涉及多表更新，需使用事务保障一致性
- 索引与约束
  - 用户名、邮箱等字段应建立唯一索引，防止重复注册
  - 外键约束确保会话与用户关联的一致性

```mermaid
classDiagram
class AuthRepository {
+createUser(data)
+login(credentials)
+resetPassword(token, newPassword)
+getSession(userId)
}
class VerificationRepository {
+generateCode(userId)
+verifyCode(code)
+expireCode(code)
}
AuthRepository --> VerificationRepository : "协作"
```

图表来源
- [src/features/auth/auth-repository.ts](file://src/features/auth/auth-repository.ts)
- [src/features/auth/verification-repository.ts](file://src/features/auth/verification-repository.ts)

章节来源
- [src/features/auth/auth-repository.ts](file://src/features/auth/auth-repository.ts)
- [src/features/auth/verification-repository.ts](file://src/features/auth/verification-repository.ts)
- [src/features/auth/account-service.pg.test.ts](file://src/features/auth/account-service.pg.test.ts)

### 导演运行时数据访问
- 运行时Repository
  - runtime-repository.ts维护管道节点状态、产物元数据、阶段推进记录
- 状态机与事务
  - 阶段推进需要原子性更新，避免中间态导致的不一致
- 查询优化
  - 针对高频查询（如按项目、阶段筛选）建立复合索引

```mermaid
sequenceDiagram
participant Runner as "阶段执行器"
participant Repo as "RuntimeRepository"
participant DB as "PostgreSQL"
Runner->>Repo : advanceStage(projectId, stage)
Repo->>DB : BEGIN
Repo->>DB : 更新节点状态
Repo->>DB : 写入产物元数据
Repo->>DB : COMMIT
DB-->>Repo : 成功
Repo-->>Runner : 新状态
```

图表来源
- [src/features/director/runtime-repository.ts](file://src/features/director/runtime-repository.ts)
- [src/features/director/runtime-repository.pg.test.ts](file://src/features/director/runtime-repository.pg.test.ts)

章节来源
- [src/features/director/runtime-repository.ts](file://src/features/director/runtime-repository.ts)
- [src/features/director/runtime-repository.pg.test.ts](file://src/features/director/runtime-repository.pg.test.ts)

### 渲染与缓存数据访问
- 渲染Repository
  - 管理渲染任务、帧序列、缩略图、媒体组装等
- 缓存Repository
  - cache.pg.test.ts体现缓存命中与失效策略
- 索引策略
  - 针对任务状态、项目ID、时间戳建立索引以加速查询

```mermaid
flowchart TD
Q["查询请求"] --> CheckCache{"缓存命中?"}
CheckCache --> |是| ReturnCache["返回缓存"]
CheckCache --> |否| QueryDB["查询数据库"]
QueryDB --> UpdateCache["更新缓存"]
UpdateCache --> ReturnResult["返回结果"]
```

图表来源
- [src/features/render/cache.pg.test.ts](file://src/features/render/cache.pg.test.ts)
- [src/features/render/repository.pg.test.ts](file://src/features/render/repository.pg.test.ts)

章节来源
- [src/features/render/cache.pg.test.ts](file://src/features/render/cache.pg.test.ts)
- [src/features/render/repository.pg.test.ts](file://src/features/render/repository.pg.test.ts)

### 路由与媒体数据访问
- 媒体路由Repository
  - media-route-repository.pg.test.ts体现媒体路由表的CRUD与查询
- 模型路由Repository
  - model-route-repository.pg.test.ts体现模型路由与版本管理
- 约束与索引
  - 路由唯一性、外键关联、时间戳索引

```mermaid
classDiagram
class MediaRouteRepository {
+createRoute(route)
+getRoute(id)
+updateRoute(id, data)
+deleteRoute(id)
}
class ModelRouteRepository {
+createModelRoute(modelRoute)
+getModelVersion(id, version)
+listVersions(id)
}
```

图表来源
- [src/features/routing/media-route-repository.pg.test.ts](file://src/features/routing/media-route-repository.pg.test.ts)
- [src/features/routing/model-route-repository.pg.test.ts](file://src/features/routing/model-route-repository.pg.test.ts)

章节来源
- [src/features/routing/media-route-repository.pg.test.ts](file://src/features/routing/media-route-repository.pg.test.ts)
- [src/features/routing/model-route-repository.pg.test.ts](file://src/features/routing/model-route-repository.pg.test.ts)

### 画布查询与视图
- 画布查询
  - canvas/queries.ts提供复杂查询与聚合逻辑，支撑前端展示与分析
- 性能考量
  - 大对象查询分页、延迟加载、只读副本

章节来源
- [src/features/canvas/queries.ts](file://src/features/canvas/queries.ts)

## 计费系统数据库架构

### 计费领域模型设计
计费系统采用领域驱动设计原则，将复杂的计费逻辑抽象为清晰的领域模型。主要实体包括：

- **工作空间权限（Workspace Permission）**：管理用户对工作空间的访问权限和控制策略
- **计费周期（Billing Period）**：定义计费的时间范围和周期规则
- **费率卡（Rate Card）**：存储不同服务类型的定价规则和折扣策略
- **合同（Contract）**：记录客户与服务提供商之间的计费协议
- **兑换交易（Redemption Transaction）**：追踪优惠券、积分等兑换活动的执行情况

```mermaid
erDiagram
WORKSPACE_PERMISSION {
uuid id PK
uuid workspace_id FK
uuid user_id FK
string permission_type
boolean is_active
timestamp created_at
timestamp updated_at
}
BILLING_PERIOD {
uuid id PK
uuid workspace_id FK
date start_date
date end_date
string period_type
boolean is_current
timestamp created_at
}
RATE_CARD {
uuid id PK
string service_type
decimal base_price
decimal discount_rate
jsonb pricing_rules
boolean is_active
timestamp created_at
timestamp updated_at
}
CONTRACT {
uuid id PK
uuid workspace_id FK
uuid provider_id FK
string contract_type
jsonb terms
decimal total_amount
string status
timestamp created_at
timestamp updated_at
}
REDEMPTION_TRANSACTION {
uuid id PK
uuid workspace_id FK
uuid user_id FK
uuid rate_card_id FK
decimal amount
string transaction_type
string status
timestamp created_at
}
WORKSPACE_PERMISSION ||--o{ USER : grants_to
WORKSPACE_PERMISSION ||--o{ WORKSPACE : belongs_to
BILLING_PERIOD ||--o{ RATE_CARD : contains
CONTRACT ||--o{ RATE_CARD : uses
REDEMPTION_TRANSACTION ||--o{ RATE_CARD : references
REDEMPTION_TRANSACTION ||--o{ USER : performed_by
```

图表来源
- [src/features/billing/domain.ts](file://src/features/billing/domain.ts)
- [src/features/billing/contracts.ts](file://src/features/billing/contracts.ts)
- [src/features/billing/rate-card.ts](file://src/features/billing/rate-card.ts)
- [src/features/billing/redemption.ts](file://src/features/billing/redemption.ts)

### 计费数据访问层
计费系统采用Repository模式隔离数据访问细节，提供统一的接口供业务层调用：

- **费率卡Repository**：rate-card-repository.ts提供费率卡的CRUD操作和查询功能
- **事务管理**：计费操作涉及多个表的原子性更新，确保数据一致性
- **查询优化**：针对计费统计、报表生成等高频查询建立专用索引

```mermaid
classDiagram
class RateCardRepository {
+createRateCard(rateCard)
+getRateCardById(id)
+updateRateCard(id, data)
+deleteRateCard(id)
+searchRateCards(filters)
+calculatePricing(serviceType, usage)
}
class BillingPeriodService {
+getCurrentPeriod(workspaceId)
+getNextPeriod(workspaceId)
+validatePeriodOverlap(periods)
+getUsageStats(workspaceId, period)
}
class RedemptionService {
+processRedemption(transaction)
+validateRedemption(transaction)
+trackTransactionStatus(transactionId)
+getRedemptionHistory(workspaceId)
}
RateCardRepository --> BillingPeriodService : "协作"
BillingPeriodService --> RedemptionService : "协调"
```

图表来源
- [src/features/billing/rate-card-repository.ts](file://src/features/billing/rate-card-repository.ts)
- [src/features/billing/period-service.ts](file://src/features/billing/period-service.ts)
- [src/features/billing/redemption.ts](file://src/features/billing/redemption.ts)

### 计费事务处理
计费系统的核心事务包括：

- **计费周期切换**：确保周期切换的原子性和一致性
- **费率计算**：基于用量和服务类型计算准确费用
- **兑换处理**：处理优惠券、积分等兑换交易的完整流程
- **账单生成**：汇总所有消费记录生成最终账单

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Billing as "计费服务"
participant Repo as "Repository"
participant DB as "PostgreSQL"
Client->>Billing : 创建计费周期
Billing->>Repo : BEGIN TRANSACTION
Repo->>DB : 插入周期记录
Repo->>DB : 设置当前周期标志
Repo->>DB : 更新相关权限
Repo->>DB : COMMIT
DB-->>Repo : 提交成功
Repo-->>Billing : 返回周期信息
Billing-->>Client : 确认创建成功
```

图表来源
- [src/features/billing/period-service.ts](file://src/features/billing/period-service.ts)
- [src/features/billing/rate-card-repository.ts](file://src/features/billing/rate-card-repository.ts)

章节来源
- [src/features/billing/domain.ts](file://src/features/billing/domain.ts)
- [src/features/billing/contracts.ts](file://src/features/billing/contracts.ts)
- [src/features/billing/rate-card.ts](file://src/features/billing/rate-card.ts)
- [src/features/billing/redemption.ts](file://src/features/billing/redemption.ts)
- [src/features/billing/period-service.ts](file://src/features/billing/period-service.ts)
- [src/features/billing/ledger.ts](file://src/features/billing/ledger.ts)
- [src/features/billing/rate-card-repository.ts](file://src/features/billing/rate-card-repository.ts)

## 依赖关系分析
数据层依赖关系清晰：应用服务依赖Repository，Repository依赖Drizzle ORM，ORM依赖PostgreSQL。迁移与初始化脚本独立于运行期，确保环境一致性。

```mermaid
graph LR
App["应用服务"] --> Repo["Repository"]
Repo --> Drizzle["Drizzle ORM"]
Drizzle --> PG["PostgreSQL"]
Init["迁移与初始化"] --> Drizzle
Backup["备份与恢复"] --> PG
Billing["计费系统"] --> BillingRepo["计费Repository"]
BillingRepo --> Drizzle
```

图表来源
- [drizzle.config.ts](file://drizzle.config.ts)
- [scripts/setup/db-migrate.ts](file://scripts/setup/db-migrate.ts)
- [scripts/migration/import-postgres.ts](file://scripts/migration/import-postgres.ts)
- [scripts/migration/sqlite-backup.ts](file://scripts/migration/sqlite-backup.ts)
- [src/features/billing/rate-card-repository.ts](file://src/features/billing/rate-card-repository.ts)

章节来源
- [drizzle.config.ts](file://drizzle.config.ts)
- [scripts/setup/db-migrate.ts](file://scripts/setup/db-migrate.ts)
- [scripts/migration/import-postgres.ts](file://scripts/migration/import-postgres.ts)
- [scripts/migration/sqlite-backup.ts](file://scripts/migration/sqlite-backup.ts)

## 性能考虑
- 查询优化
  - 使用EXPLAIN ANALYZE分析慢查询，避免SELECT *，仅选择必要字段
  - 合理使用JOIN与子查询，必要时物化视图
  - **新增** 计费相关查询优化：预聚合统计数据、缓存热点费率卡信息
- 索引设计
  - 为高频过滤字段（如project_id、status、created_at）建立B-tree索引
  - 复合索引覆盖常见查询组合，避免回表
  - **新增** 计费索引策略：workspace_id + period_type复合索引、service_type + is_active索引
- 连接池管理
  - 合理设置max connections与idle timeout，避免连接泄漏
- 分库分表
  - 按项目或时间维度拆分大表，读写分离与归档策略
  - **新增** 计费数据按时间分区，历史账单自动归档
- 缓存策略
  - 热点数据Redis缓存，失效策略与一致性保证
  - **新增** 费率卡缓存、计费周期状态缓存

## 故障排查指南
- 迁移失败
  - 检查drizzle.config.ts配置与数据库连通性
  - 查看db-migrate.ts日志定位错误
- 数据不一致
  - 使用reconcile-postgres.ts进行一致性校验与修复
- 备份恢复
  - 使用sqlite-backup.ts进行本地备份，import-postgres.ts进行恢复
- 主密钥问题
  - 使用provision-master-key.ts重新生成或导入密钥
- **新增** 计费系统问题排查
  - 检查计费周期状态和数据一致性
  - 验证费率卡配置的有效性和可用性
  - 监控兑换交易的完整性和准确性
  - 分析计费计算的准确性和时效性

章节来源
- [drizzle.config.ts](file://drizzle.config.ts)
- [scripts/setup/db-migrate.ts](file://scripts/setup/db-migrate.ts)
- [scripts/migration/reconcile-postgres.ts](file://scripts/migration/reconcile-postgres.ts)
- [scripts/migration/sqlite-backup.ts](file://scripts/migration/sqlite-backup.ts)
- [scripts/migration/import-postgres.ts](file://scripts/migration/import-postgres.ts)
- [scripts/migration/provision-master-key.ts](file://scripts/migration/provision-master-key.ts)

## 结论
PurpleInk的数据库设计以PostgreSQL为核心，结合Drizzle ORM实现类型安全的Schema与查询。Repository模式隔离了数据访问细节，便于测试与维护。通过合理的索引、事务与连接池管理，系统在性能与一致性之间取得平衡。备份与恢复脚本提供了可靠的运维能力。未来可进一步引入缓存、读写分离与分库分表以提升扩展性。

**更新** 新增的计费系统数据库架构为平台提供了完整的商业化支持，包括灵活的工作空间权限管理、精确的计费周期控制、多样化的费率卡配置、完善的合同管理和透明的兑换交易追踪。这些功能共同构成了一个健壮、可扩展的计费基础设施。

## 附录
- 实体关系图（概念）
```mermaid
erDiagram
USER {
uuid id PK
string email UK
string username UK
timestamp created_at
timestamp updated_at
}
SESSION {
uuid id PK
uuid user_id FK
string token
timestamp expires_at
}
VERIFICATION_CODE {
uuid id PK
uuid user_id FK
string code
timestamp expires_at
}
RUNTIME_NODE {
uuid id PK
uuid project_id FK
string stage
jsonb state
timestamp updated_at
}
RENDER_TASK {
uuid id PK
uuid project_id FK
enum status
timestamp created_at
}
MEDIA_ROUTE {
uuid id PK
string path
uuid source_id FK
timestamp created_at
}
MODEL_ROUTE {
uuid id PK
string name
int version
timestamp created_at
}
WORKSPACE_PERMISSION {
uuid id PK
uuid workspace_id FK
uuid user_id FK
string permission_type
boolean is_active
timestamp created_at
}
BILLING_PERIOD {
uuid id PK
uuid workspace_id FK
date start_date
date end_date
string period_type
boolean is_current
timestamp created_at
}
RATE_CARD {
uuid id PK
string service_type
decimal base_price
decimal discount_rate
jsonb pricing_rules
boolean is_active
timestamp created_at
}
CONTRACT {
uuid id PK
uuid workspace_id FK
uuid provider_id FK
string contract_type
jsonb terms
decimal total_amount
string status
timestamp created_at
}
REDEMPTION_TRANSACTION {
uuid id PK
uuid workspace_id FK
uuid user_id FK
uuid rate_card_id FK
decimal amount
string transaction_type
string status
timestamp created_at
}
USER ||--o{ SESSION : has
USER ||--o{ VERIFICATION_CODE : generates
PROJECT ||--o{ RUNTIME_NODE : contains
PROJECT ||--o{ RENDER_TASK : triggers
MEDIA_ROUTE ||--o{ ARTIFACT : points_to
MODEL_ROUTE ||--o{ VERSION : tracks
WORKSPACE_PERMISSION ||--o{ USER : grants_to
BILLING_PERIOD ||--o{ RATE_CARD : contains
CONTRACT ||--o{ RATE_CARD : uses
REDEMPTION_TRANSACTION ||--o{ RATE_CARD : references
```

[此图为概念性ER图，未映射到具体源码文件]