...
- 优化了错误传播链路的可观测性和安全性
- **新增**：计费配额限制的402错误处理机制，通过现有对话框基础设施提供适当的用户反馈和升级指导
...
**最新更新**：系统已增强OpenAI SDK获取边界的错误观测能力，实现了提供商响应的安全流错误清理机制，并改进了DirectorRunError实例以支持更准确的工作流分类。**新增了计费配额限制的402错误处理机制，能够正确处理真实的配额耗尽错误，并通过现有对话框基础设施为用户提供适当的反馈和升级指导**。
...
F5["billing/quota-handler"]
end
subgraph "工具层"
L1["error-message"]
L2["logger"]
L3["lib/api"]
L4["dialog-infrastructure"]
end
R1 --> F1
R2 --> F1
R3 --> F1
...
- [src/app/api/director/pipeline/route.ts](file://src/app/api/director/pipeline/route.ts)
- [src/app/api/jobs/[id]/route.ts](file://src/app/api/jobs/[id]/route.ts)
- [src/app/api/billing/route.ts](file://src/app/api/billing/route.ts)
- [src/features/auth/errors.ts](file://src/features/auth/errors.ts)
- [src/features/canvas/workflow-error.ts](file://src/features/canvas/workflow-error.ts)
- [src/features/director/artifact-validation-error.ts](file://src/features/director/artifact-validation-error.ts)
- [src/features/ai/route-contract-error.ts](file://src/features/ai/route-contract-error.ts)
...
- **新增**：计费配额限制处理器，专门处理402错误并提供用户友好的升级指导。
...
- **计费相关**：配额耗尽、订阅过期、支付失败等，提供明确的升级指引。

**更新**：DirectorRunError实例现已改进以支持更准确的工作流分类，提供更好的错误处理和诊断能力。**新增了专门的计费错误类型，用于处理配额限制和订阅相关问题**。

建议：
- 错误码采用"模块前缀+具体场景"的结构，例如AUTH_001、WORKFLOW_002、BILLING_001。
- 错误码与人类可读消息分离，便于多语言与国际化。
- 对于402错误，应包含具体的配额信息和升级选项。
...
- **402错误特殊字段**：quotaInfo、upgradeLink、currentPlan、limitReached等。

**更新**：流错误清理机制确保敏感信息不会泄露到错误响应中，提高了系统的安全性。**402错误现在包含详细的配额信息和升级指导，帮助用户快速解决问题**。

建议：
- 错误消息对用户友好，不包含敏感信息；调试信息通过traceId关联日志。
- 对批量操作返回部分失败时，使用数组形式的details描述每个子项结果。
- 对于配额相关的错误，提供具体的使用量、限制值和升级选项。
...
- **配额限制错误**：专门处理402错误，通过对话框基础设施提供用户反馈。

**更新**：OpenAI SDK边界错误观测器提供了增强的错误分类能力，能够更准确地识别和分类不同类型的错误。**新增了配额限制错误的专门处理流程，确保402错误能够正确传播到用户界面**。

传播建议：
- 路由层捕获并转换异常为统一错误响应。
- 特征模块抛出领域错误，由上层统一处理。
- 工具层负责格式化与日志记录，避免侵入业务逻辑。
- 配额错误应优先处理，及时中断业务流程并引导用户升级。
...
- **402错误处理**：显示升级对话框，提供订阅管理入口。

**更新**：增强的错误观测能力使客户端能够更准确地识别错误类型并应用相应的处理策略。**新增了针对402配额耗尽错误的专门处理逻辑，通过现有对话框基础设施为用户提供无缝的升级体验**。
...
**更新**：OpenAI SDK边界观测器提供了更详细的错误上下文信息，有助于快速定位和解决问题。**配额限制相关的错误现在包含详细的用户信息和配额状态，便于分析和监控**。
...
### 计费配额限制处理
**新增功能**：实现了专门的计费配额限制错误处理机制。

主要特性：
- 402错误识别：准确识别配额耗尽的HTTP 402错误
- 用户反馈：通过现有对话框基础设施提供友好的错误提示
- 升级指导：提供清晰的升级路径和订阅管理入口
- 配额信息：显示当前使用情况、限制值和剩余配额
- 错误传播：确保配额错误在整个系统中正确传播和处理
...
**更新**：OpenAI SDK边界观测器和流错误清理机制的引入增加了新的依赖关系，但保持了良好的解耦性。**新增的配额限制处理器与对话框基础设施集成，提供了完整的用户交互体验**。
...
Feature --> QuotaHandler["配额限制处理器"]
QuotaHandler --> Dialog["对话框基础设施"]
SDKObserver --> Log
...
**更新**：OpenAI SDK边界观测器采用了轻量级的设计模式，最小化了对性能的影响。流错误清理机制也经过优化，确保在错误情况下的高效处理。**配额限制处理器设计为快速失败模式，在检测到配额耗尽时立即返回，避免不必要的资源消耗**。
...
**更新**：增强的错误观测能力提供了更详细的诊断信息，包括SDK级别的错误详情和流传输过程中的错误上下文，大大简化了问题定位过程。**配额限制相关的错误现在包含详细的用户信息和配额状态，便于分析配额使用模式和监控配额耗尽事件**。
...
## 结论
通过统一的状态码规范、清晰的错误码定义、标准化的错误响应体与结构化日志，结合客户端的重试与降级策略，可以显著提升系统的可观测性与用户体验。**最新的OpenAI SDK边界错误观测和流错误清理机制进一步增强了系统的健壮性和安全性，而新增的配额限制处理机制则为计费相关的错误提供了专业的处理方式**。建议在新增功能时同步完善错误处理与测试用例，确保错误路径的可维护性与稳定性。
