...
function executeQualityGate(bundleRoot, reportPath, renderDirectory) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [qualityScript, bundleRoot, reportPath, renderDirectory], { stdio: ["ignore", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
...
async function defaultRun(bundleRoot, reportPath, renderDirectory) {
  await executeQualityGate(bundleRoot, reportPath, renderDirectory);
...
export async function hyperframesQualityGate({ bundle }, dependencies = {}) {
  ...
  const renderDirectory = join(directory, "renders");
  try {
    await writeBundle(bundle, directory);
    const report = await (dependencies.run ?? defaultRun)(directory, reportPath, renderDirectory);
    ...
    const previewBytes = await readFile(join(renderDirectory, "landscape.mp4"));
    return { report, previewBytes };
  } finally {
