...
export async function runCli(
  ...
  try {
    ...
    output.writeLine(args?.json ? JSON.stringify(payload) : `失败（${code}）。请运行 purpleink-video doctor 检查本地依赖。`)
    return 1
  }
}

function formatHumanResult(value: unknown): string {
  if (typeof value === 'object' && value !== null && 'usage' in value && typeof value.usage === 'string') return value.usage
  if (isRecord(value) && typeof value.command === 'string') {
    if (value.command === 'run') return `run ${String(value.status)}\nrunDir: ${String(value.runDir)}\nvideo: ${String(value.videoPath)}`
    if (value.command === 'plan') return `plan completed\nrunDir: ${String(value.runDir)}\nshots: ${String(value.shots instanceof Array ? value.shots.length : 0)}`
    if (value.command === 'status') return `status ${isRecord(value.run) ? String(value.run.status) : 'unknown'}\nrunDir: ${isRecord(value.run) ? String(value.run.runDir) : 'unknown'}`
...
const directCliExecution = process.argv.some((value) => /(?:^|[\\/])src[\\/]cli\.ts$/u.test(value)) && process.env.VITEST !== 'true'
