...
describe('observer', () => {
  it('serves registered artifacts and rejects unregistered paths', async () => {
    ...
    const videoPath = join(run.runDir, 'final', 'video.mp4')
    await mkdir(join(run.runDir, 'final'), { recursive: true })
    await writeFile(videoPath, 'test-video', 'utf8')
    await registerFileArtifact(store, run.runDir, { id: 'video', kind: 'video/mp4', path: videoPath })
    ...
    try {
      ...
      expect(homeSource).toContain('观察页测试')
      expect(homeSource).toContain('等待 Agent 验收')
      const runPage = await fetch(`${observer.url}run/run-observer`)
      const runSource = await runPage.text()
      expect(runSource).toContain('MP4 已生成，等待 Agent 最终验收')
      expect(runSource).toContain('可以立即预览')
