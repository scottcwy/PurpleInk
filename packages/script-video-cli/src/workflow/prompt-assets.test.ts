...
describe('prompt assets', () => {
  ...
  it('keeps existing DIRECT and SHOT-SPEC builders and adds the FABRICATE builder', () => {
    ...
    expect(buildShotSpecPrompt(input, director, input.units[0]!, 'S001').user).toContain('whoosh-soft')
    expect(buildShotSpecPrompt(input, director, input.units[0]!, 'S001').user).toContain('soundEffects')
    ...
    expect(buildFabricatePrompt(input, input.units[0]!, shot).user).toContain('HTML 内禁止创建 audio')
