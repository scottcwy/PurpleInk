...
function createShotTimelineRegistration(id: string, durationSec: number): string {
  return `window.__timelines=window.__timelines||{};(function(render){if(!render||!render.timeline){console.warn("[PurpleInk] shot timeline missing",${JSON.stringify(id)});return}var timeline=render.timeline;var targetDuration=${round(durationSec)};var sourceDuration=typeof timeline.duration==="function"?timeline.duration():0;function stretch(animation,ratio){var children=typeof animation.getChildren==="function"?animation.getChildren(false,true,true):[];if(children.length){children.forEach(function(child){if(typeof child.startTime==="function")child.startTime(child.startTime()*ratio);stretch(child,ratio)});if(typeof animation.invalidate==="function")animation.invalidate()}else if(typeof animation.duration==="function"){animation.duration(animation.duration()*ratio)}if(typeof animation.repeatDelay==="function"&&animation.repeatDelay()>0)animation.repeatDelay(animation.repeatDelay()*ratio)}if(sourceDuration>0&&targetDuration>0&&Math.abs(sourceDuration-targetDuration)>0.0001)stretch(timeline,targetDuration/sourceDuration);if(typeof timeline.timeScale==="function")timeline.timeScale(1);if(typeof timeline.pause==="function")timeline.pause();render.durationSec=targetDuration;window.__timelines[${JSON.stringify(id)}]=timeline})(window.__PURPLEINK_RENDER__);`
}
